import { Timestamp } from 'firebase/firestore';
import { AiTwotoneTags } from 'react-icons/ai';
import { DateTime } from '@/components/common/Date/DateTime';
import { Image } from '@/components/common/Image';
import { Link } from '@/components/common/Link';

type Props = {
  title: string;
  thumbnail: string;
  releaseDate: Timestamp;
  tags: string[];
};

export const PostHeader = ({ title, thumbnail, releaseDate, tags }: Props) => {

  return (
    <div className="vstack gap-4">
      <div className="w-full h-64 sm:h-80">
        <Image
          src={thumbnail}
          alt={`thumbnail for ${title}`}
          className="w-full h-full object-contain"
        />
      </div>
      <h1 className="text-primary-1 text-3xl md:text-4xl font-normal tracking-tighter leading-tight">
        {title}
      </h1>
      <div className="wrap gap-4">
        <DateTime date={releaseDate} />
        <div className="wrap gap-2">
          <span className="select-none text-primary-1">
            <AiTwotoneTags />
          </span>
          {tags.map((tag) => (
            <Link key={tag} href={`/tags/${tag}`} passHref>
              <a className="badge">{tag}</a>
            </Link>
          ))}
        </div>
      </div>
    </div>
  );
};
